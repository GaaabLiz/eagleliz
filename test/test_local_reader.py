...
def test_reader_scan_basic():
    ...
    # Use include_deleted=True because the test catalog items are mostly marked as deleted
    reader = EagleLocalReader(TEST_CATALOG_PATH, include_deleted=True)
    reader.run()
...
def test_reader_filter_tags():
    ...
    reader = EagleLocalReader(
        TEST_CATALOG_PATH, include_deleted=True, filter_tags=["BatchURL"]
    )
    reader.run()
