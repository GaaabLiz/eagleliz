...
import os
import time
from eagleliz.api.eagleapi import (
    EagleItemURLPayload,
...
class TestItemAPI:
    ...
    def test_add_items_from_urls_batch(self, api):
        item1 = EagleItemURLPayload(
            url="https://picsum.photos/id/1/100", name="Batch_1", tags=["BatchURL"]
        )
        item2 = EagleItemURLPayload(
            url="https://picsum.photos/id/2/100", name="Batch_2", tags=["BatchURL"]
        )
        success = api.add_items_from_urls([item1, item2], folder_id=self.sandbox.id)
        assert success is True
        assert wait_for_items(
            api, tags=["BatchURL"], folders=[self.sandbox.id], expected_count=2
        ), "Batch items not found"
    ...
    def test_add_item_from_path(self, api, ext, content):
        ...
        try:
            ...
            assert success is True
            assert wait_for_items(api, keyword=name, folders=[self.sandbox.id]), (
                f"Path item {name} not found"
            )
    ...
    def test_add_items_from_paths_batch(self, api):
        with (
            tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as t1,
        ...
        try:
            ...
            assert success is True
            assert wait_for_items(
                api, tags=["BPath"], folders=[self.sandbox.id], expected_count=2
            ), "Batch path items not found"
    ...
    # ==========================
    @pytest.mark.skip(
        reason="Eagle bookmark indexing is too slow/asynchronous for reliable testing"
    )
    def test_add_bookmark(self, api):
        ...
        success = api.add_bookmark(
            url="https://pytest.org", name=name, base64=dummy_base64_png, tags=[bm_tag]
        )
        assert success is True

        # 1. Wait via API
        assert wait_for_items(api, tags=[bm_tag], timeout=60), (
            f"Bookmark with tag {bm_tag} not found via API"
        )
        ...
        assert info_dir.exists() and info_dir.is_dir(), (
            f"Bookmark info directory {info_dir} not found"
        )
    ...
    def test_get_update_refresh_cycle(self, api):
        ...
        updated = api.update_item(
            item_id=item_id, tags=["UpdatedCycle"], annotation="Updated Note", star=3
        )
    ...
    @pytest.mark.parametrize(
        "order_by",
        ["NAME", "-NAME", "FILESIZE", "-FILESIZE", "CREATEDATE", "-CREATEDATE"],
    )
    ...
    def test_move_to_trash(self, api):
        api.add_item_from_url(
            "https://picsum.photos/id/66/10", name="TrashMe", folderId=self.sandbox.id
        )
        assert wait_for_items(api, keyword="TrashMe", folders=[self.sandbox.id]), (
            "TrashMe item not found"
        )
