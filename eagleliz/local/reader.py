...
from eagleliz.model.local import EagleLocalItem
from eagleliz.model.metadata import Metadata


class EagleLocalReader:
    """
    ...
    Attributes:
        ...
        items (List[EagleLocalItem]): Accepted list of populated, mapped Eagle items.
        error_paths (List[Tuple[Path, str]]): Paths that raised filesystem or parsing errors.
        items_skipped (List[Tuple[EagleLocalItem, str]]): Valid items filtered down during scan.
    ...
    def __init__(
        ...
        self.items_skipped: List[Tuple[EagleLocalItem, str]] = []
    ...
    def __handle_eagle_folder(self, folder: Path) -> Optional[EagleLocalItem]:
        """
        ...
        Returns:
            Optional[EagleLocalItem]: Nullable EagleLocalItem initialized based on `metadata.json` mapping. None if failed/filtered.
        """
        ...
        eagle_item = EagleLocalItem(media_file, metadata_obj)
